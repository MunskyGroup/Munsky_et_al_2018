function [Dynamics,N_Space] = hog_model_FSP_TS_class(obj)
...
%% First, we find the gene-state distributions at times previous to the measurements.
PARS.N_Space = [obj.Num_States 0 0];
PARS.Time_Array = obj.tt*60 - obj.gene_length/obj.k_elong;
PARS.N_states=PARS.N_Space(1)*...
    (PARS.N_Space(2)+1)*...
    (PARS.N_Space(3)+1);  % Maximum number of states.
Model.Pars_Degrade_Nuc.Const = 0;
Model.Pars_Degrade_Nuc.Time = 0;
Model.Pars_Degrade_Cyt.Const = 0;
Model.Pars_Degrade_Cyt.Time = 0;
Model.Pars_Transport.Const = 0;
Model.Pars_Transport.Time = 0;
...
PARS.t_offset = 0;
[~,states_in_past] = PARS.HISTOGRAMS(PARS,Model,Reactions.Hog_Parameters);


%% Next, we integrate from the previous times to determine the Distribution of nascent lengths over time.
PARS.N_Space = [obj.Num_States obj.TS_Lim 0];
PARS.N_states=PARS.N_Space(1)*...
    (PARS.N_Space(2)+1)*...
    (PARS.N_Space(3)+1);  % Maximum number of states.
PARS = cme_General_Spatial_NState(PARS);  %% This function will define and store the infintesimal generator of the master equation.
PARS.Degrade=[0 0];
PARS.Transport = 0;
Dynamics = zeros(PARS.N_Space(2)+1,length(obj.tt));
for it = 1:length(obj.tt)
    PARS.Time_Array = linspace(obj.tt(it)*60-obj.gene_length/obj.k_elong,obj.tt(it)*60,3);
    P0 = spalloc(PARS.N_states,1,PARS.N_Space(1));
    P0(1:PARS.N_Space(1),1) = states_in_past(it,:)';
    Model.Pars_Degrade_Nuc.Const = 0;
    Model.Pars_Degrade_Nuc.Time = 0;
    Model.Pars_Degrade_Cyt.Const = 0;
    Model.Pars_Degrade_Cyt.Time = 0;
    Model.Pars_Transport.Const = 0;
    Model.Pars_Transport.Time = 0;
    TS_Dynamics = PARS.HISTOGRAMS(PARS,Model,Reactions.Hog_Parameters,P0);
    Dynamics(:,it) = TS_Dynamics(end,:)';
end
